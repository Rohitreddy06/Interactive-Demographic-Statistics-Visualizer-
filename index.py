...
if df[columns[2]].dtype == 'object':
    st.warning("Selected value column is categorical. Converting to numerical representation.")
...
time_col = st.selectbox("Select time column for forecasting", options=columns)
...
if st.button("Run Forecasting"):
    ...
    st.write(forecast)
    fig, ax = plt.subplots()
    ax.plot(df[time_col], df[target_col], label="Actual")
    ax.plot(pd.date_range(df[time_col].iloc[-1], periods=10, freq='M'), forecast, label="Forecast", linestyle='dashed')
    ax.legend()
    st.pyplot(fig)